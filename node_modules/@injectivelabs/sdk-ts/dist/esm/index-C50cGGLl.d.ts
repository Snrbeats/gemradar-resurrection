...
/**
 * BaseIndexerGrpcConsumer extends BaseGrpcConsumer for Indexer APIs.
 * It provides the same functionality as BaseGrpcConsumer but is specifically
 * designed for Indexer gRPC APIs.
 */
declare class BaseIndexerGrpcConsumer extends BaseGrpcConsumer {}
//#endregion
//#region src/client/olp/grpc/OLPGrpcApi.d.ts
declare class OLPGrpcApi extends BaseIndexerGrpcConsumer {
  protected module: string;
  private get client();
  fetchEpochs(status?: string): Promise<EpochV2[]>;
  fetchMarketRewards(epochId: string): Promise<MarketReward[]>;
  ...
  fetchRewardsEligibility({
    epochId,
    accountAddress
  }: {
    epochId?: string;
    accountAddress?: string;
  }): Promise<RewardsEligibility>;
  fetchMarketRewardsRange({
    epochId,
    marketId
  }: {
    epochId: string;
    marketId?: string;
  }): Promise<MinMaxRewards>;
}
//#endregion
//#region src/client/olp/grpc/types/index.d.ts
interface DmmPagination {
  from?: number;
  height?: number;
  perPage?: number;
  total?: number;
...
interface MinMaxRewards {
  minRewards: Record<string, string>;
  maxRewards: Record<string, string>;
}
type GrpcEpochsV2 = GetEpochsResponse;
type GrpcEpochV2 = EpochV2$1;
type GrpcEpochConfigV2 = EpochConfigV2$1;
type GrpcMarketReward = MarketReward$1;
type GrpcMarketRewards = GetMarketRewardsResponse;
type GrpcDmmPagination = Pagination;
type GrpcEligibleAddresses = GetEligibleAddressesResponse;
type GrpcEligibleAddress = EligibleAddress$1;
type GrpcEpochScores = GetEpochScoresResponse;
type GrpcEpochScore = EpochScore$1;
type GrpcEpochScoresHistory = GetEpochScoresHistoryResponse;
type GrpcTotalScores = GetTotalScoresHistoryResponse;
type GrpcTotalScore = TotalScore$1;
type GrpcTotalScoresHistory = GetTotalScoresHistoryResponse;
type GrpcRewardsDistribution = GetRewardsDistributionResponse;
type GrpcRewardDistribution = RewardDistribution$1;
type GrpcAccountVolume = AccountVolume$1;
type GrpcRewardsEligibility = GetRewardsEligibilityResponse;
//#endregion
